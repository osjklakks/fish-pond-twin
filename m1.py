...
import os
DIR = r'D:\夸克\3'
fname = os.path.join(DIR, 'merged.html')
...
:root {
  ...
  --accent-purple:#a78bfa;--accent-purple-dim:rgba(167,139,250,.12);
...
.btn-accent{background:var(--accent-orange);color:#000;border-color:var(--accent-orange);font-weight:600}
...
/* Glass card (from file1, adapted to file2 colors) */
.glass-card{background:rgba(17,24,39,.85);backdrop-filter:blur(12px);border:1px solid rgba(34,211,238,.15);border-radius:var(--radius-lg);transition:all var(--transition)}
.glass-card:hover{border-color:rgba(34,211,238,.3)}

/* Hero */
.hero-section{min-height:100vh;display:flex;align-items:center;justify-content:center;padding:80px 24px 40px;position:relative;overflow:hidden}
.hero-content{max-width:900px;text-align:center}
.hero-badge{display:inline-block;padding:6px 16px;background:var(--accent-cyan-dim);color:var(--accent-cyan);border-radius:20px;font-size:13px;font-weight:500;margin-bottom:20px}
.hero-title{font-size:56px;font-weight:700;margin-bottom:20px;line-height:1.1;background:linear-gradient(135deg,var(--text-primary),var(--accent-cyan));-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text}
.hero-desc{font-size:18px;color:var(--text-secondary);line-height:1.7;margin-bottom:32px;max-width:640px;margin-left:auto;margin-right:auto}
.hero-btns{display:flex;gap:16px;justify-content:center;margin-bottom:48px;flex-wrap:wrap}
.hero-btn{padding:14px 28px;border-radius:var(--radius-lg);font-size:15px;font-weight:600;cursor:pointer;transition:all var(--transition);display:inline-flex;align-items:center;gap:8px;text-decoration:none}
.hero-btn-primary{background:var(--accent-green);color:#000;border:none}
.hero-btn-primary:hover{background:#16a34a;transform:translateY(-2px);box-shadow:0 8px 24px rgba(34,197,94,.25)}
.hero-btn-outline{background:transparent;color:var(--text-secondary);border:1px solid var(--border)}
.hero-btn-outline:hover{border-color:var(--accent-cyan);color:var(--text-primary)}
.hero-stats{display:grid;grid-template-columns:repeat(4,1fr);gap:16px;max-width:640px;margin:0 auto}
.hero-stat{text-align:center;padding:20px 16px;background:var(--bg-card);border:1px solid var(--border);border-radius:var(--radius-lg)}
.hero-stat-val{font-family:var(--font-data);font-size:28px;font-weight:700;color:var(--accent-cyan)}
.hero-stat-label{font-size:12px;color:var(--text-muted);margin-top:4px}
.hero-bg-glow{position:absolute;width:600px;height:600px;border-radius:50%;filter:blur(120px);opacity:.08;pointer-events:none}
.hero-bg-glow.g1{background:var(--accent-cyan);top:-200px;right:-100px}
.hero-bg-glow.g2{background:var(--accent-green);bottom:-200px;left:-100px}

/* Progress bar */
.progress-bar{height:6px;background:var(--bg-surface);border-radius:3px;overflow:hidden;margin-top:8px}
.progress-fill{height:100%;border-radius:3px;transition:width .3s}

/* Report card */
.report-card{background:var(--bg-card);border:1px solid var(--border);border-radius:var(--radius-lg);padding:20px;cursor:pointer;transition:all var(--transition)}
...
.report-card-icon{width:44px;height:44px;border-radius:var(--radius-lg);display:flex;align-items:center;justify-content:center}
.report-card-icon i{width:22px;height:22px}
.report-card-title{font-size:16px;font-weight:600}
.report-card-desc{font-size:12px;color:var(--text-muted)}
.report-card-footer{display:flex;justify-content:space-between;align-items:center;font-size:13px}

/* Footer */
.footer{padding:48px 24px 24px;border-top:1px solid var(--border);background:var(--bg-base);margin:0 -24px -24px}
.footer-grid{display:grid;grid-template-columns:2fr 1fr 1fr 1fr;gap:32px;max-width:1100px;margin:0 auto}
.footer-brand{display:flex;align-items:center;gap:10px;margin-bottom:12px}
.footer-brand svg{width:24px;height:24px;color:var(--accent-cyan)}
...
.footer-col a{font-size:13px;color:var(--text-muted);text-decoration:none;transition:color var(--transition)}
.footer-col a:hover{color:var(--text-primary)}
.footer-bottom{text-align:center;padding-top:24px;margin-top:24px;border-top:1px solid var(--border);font-size:12px;color:var(--text-muted)}

/* Chart.js container */
.chart-js-wrap{position:relative;height:200px}
...
/* Responsive */
@media(max-width:1200px){.cards-4{grid-template-columns:repeat(2,1fr)}.pond-grid{grid-template-columns:repeat(2,1fr)}.twin-container{grid-template-columns:1fr}.twin-sidebar{flex-direction:row;overflow-x:auto}.hero-title{font-size:40px}.hero-stats{grid-template-columns:repeat(2,1fr)}.footer-grid{grid-template-columns:repeat(2,1fr)}}
@media(max-width:768px){.sidebar{width:60px}.sidebar-logo span,.nav-item span,.nav-group-label{display:none}.nav-item{justify-content:center;padding:12px}.main{margin-left:60px}.cards-4,.cards-3{grid-template-columns:1fr}.pond-grid{grid-template-columns:1fr}.hero-title{font-size:32px}.hero-stats{grid-template-columns:1fr 1fr}.footer-grid{grid-template-columns:1fr}}
"""
...
with open(fname, 'w', encoding='utf-8') as f:
    ...
    f.write('<title>智氧兴渔 - 智能鱼塘监控系统</title>\n')
    ...
    f.write('<style>\n' + css_vars + css_base + '\n</style>\n')
    f.write('</head>\n')
    print('Merged Part 1: head+CSS written,', os.path.getsize(fname), 'bytes')
